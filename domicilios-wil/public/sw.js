/* ═══════════════════════════════════════════════════════════
   ...
   UBICACIÓN: raíz del proyecto (mismo nivel que domi-panel.html)
   ═══════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════
   POLL PRINCIPAL — se ejecuta cada 5 min
   en background (cuando la app está cerrada)
   ════════════════════════════════════════════ */
async function pollPedidosBackground() {
  ...
  try {
    ...
    filas.forEach(cols => {
      ...
      /* Solo IDs numéricos válidos, sin duplicar filas del mismo pedido */
      ...
      /* Asignado a mí y todavía no notificado */
    ...
    /* 1. Notificar pedidos asignados nuevos (siempre) */
    ...
    /* 2. Notificar pendientes SOLO si el número SUBIÓ
          y la app NO está en foreground */
    ...
    /* Guardar conteo para comparar en la próxima vuelta */
    await dbSet('ultimo_conteo_pendientes', pendientesSinAsignar);

    /* Avisarle a la app abierta el resultado del poll */
    await _broadcast({
      type:           'BG_POLL_RESULT',
...
/* ─── Notificación: pedidos pendientes sin asignar ─── */
...
/* ─── Programador del poll background (cada 5 min) ─── */
...
function agendarPoll() {
  ...
  _pollTimeout = setTimeout(async () => {
    ...
    agendarPoll(); /* se reagenda solo */
  }, 5 * 60 * 1000);
}

/* ─── Fetch: pass-through (sin cache offline por ahora) ─── */
